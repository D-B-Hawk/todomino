import { getTodosWhereKey, orWhereIndexOrPrimary, sortTodosByKey } from "@/db";
...
export function getTodosCollectionByListName(listName: ListName) {
  ...
  if (listName === "todomino") {
    const todos = getTodosWhereKey("dependentTodo").notEqual("");
    return orWhereIndexOrPrimary("dependsOnTodo", todos).notEqual("");
  }
  if (listName === "today") {
